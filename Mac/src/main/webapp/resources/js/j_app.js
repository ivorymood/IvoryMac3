...
var createForm=x=>{
    return '<form id="'+x.id+'" action="'+x.action+'" class="'+x.clazz+'" method="'+x.method+'"></form>';
}
					...
					.append($(createDiv({id:'',clazz:''}))
							...
							.append($(createDiv({id:'',clazz:''}))
								.append($(createDiv({id:'',clazz:''}))
									...
									.append($(createDiv({id:'order-container', clazz:''}))//////현유 수정중
											.append($(createDiv({id:'order-row-container',clazz:''}))
													.append($(createDiv({id:'o-item-pic',clazz:''})).attr('style','display: inline-block; float: left;')
															.append($(createImg({img:'item0.jpg'}))))
													.append($(createDiv({id:'o-item-exp',clazz:''})).attr('style','display: inline-block; margin-left: 30px;')
															.append($(createDiv({id:'o-item-name',clazz:''}))
																	.append($(createHTag({num:'3',val:'롤러휠 리퀴드 라이너'}))))
															.append($(createDiv({id:'o-item-namee',clazz:''}))
																	.append('<p>ROLLERWHEEL LIQUID LINER</p>'))
															.append($(createDiv({id:'o-item-g',clazz:''}))
																	.append('<p>1 ml / 0.03 US oz</p>'))
															.append($(createDiv({id:'o-item-c',clazz:''}))
																	.append('<p>롤링 블랙 샤인</p>')))
													.append($(createDiv({id:'o-item-price',clazz:''})).attr('style','display: inline-block;  margin-left: 100px; ')
															.append('<p>정상가:₩ 35,500</p>')
															.append('<p>할인:₩ 25,500</p>'))
													.append($(createDiv({id:'o-item-count',clazz:''})).attr('style','display: inline-block;')
															.append($(createDiv({id:'',clazz:''})).attr('style','margin-left: 70px;')
																	.append($(createHTag({num:'3',val:'1pcs'})))))
													.append($(createDiv({id:'o-item-subtotal',clazz:''})).attr('style','display: inline-block;')
															.append($(createDiv({id:'',clazz:''})).attr('style','margin-left: 80px;')
